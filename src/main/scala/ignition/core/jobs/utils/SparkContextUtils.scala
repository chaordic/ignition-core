    ...
    private def readSmallFiles(smallFiles: List[HadoopFile],
      ...
      smallPartitionedFiles.mapPartitions { files =>
        ...
        files.map { case (path, _) => path } flatMap { path =>
          ...
          try {
            Source.fromInputStream(inputStream)(Codec.UTF8).getLines().filter(_.contains("tokstok")).foldLeft(ArrayBuffer.empty[String])(_ += _)
          } catch {
            case NonFatal(ex) =>
      ...
      partitionedSlices.mapPartitions { slices =>
        ...
        slices.flatMap { case (slice, _) =>
          try {
            ...
            val lines = Source.fromInputStream(inputStream)(Codec.UTF8).getLines().filter(_.contains("tokstok"))
